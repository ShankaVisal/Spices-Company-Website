...
export function CartSheet({ children }: { children?: React.ReactNode }) {
  ...
  const CartTrigger = children ? (
    ...
    <SheetTrigger asChild>
      <Button
        className="fixed bottom-6 right-4 h-14 w-14 md:bottom-6 md:right-6 md:h-16 md:w-16 rounded-full shadow-lg z-50 flex items-center justify-center bg-accent hover:bg-accent/90"
        size="icon"
